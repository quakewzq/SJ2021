...
int ask_me6()
{
        ...
        if(ob->query_temp("wd/wd_jobok") ){        	
            ...
            if(ob->query("bonus/武当") < 1)
                TASKREWARD_D->get_reward(ob,"武当",shen,0,0,5+random(2),0,0,0,"宋远桥");
            else
			{
				/*
				if (ob->query("bonus/武当") < 6) 
					TASKREWARD_D->get_reward(ob,"武当",shen,0,0,10+ random(3),0,0,0,"宋远桥");
				else
					{
						if (ob->query("bonus/武当") < 8) 
							TASKREWARD_D->get_reward(ob,"武当",shen,0,0,12+ random(3),0,0,0,"宋远桥");
						else
						{
							if (ob->query("bonus/武当") < 9) 
								TASKREWARD_D->get_reward(ob,"武当",shen,0,0,14+ random(3),0,0,0,"宋远桥");
							else
							
								TASKREWARD_D->get_reward(ob,"武当",shen,0,0,60+ random(3),0,0,0,"宋远桥");
						}
					}
				*/
				switch ((int)ob->query("bonus/武当"))
				{
					case 0:
						TASKREWARD_D->get_reward(ob,"武当",shen,0,0,3+random(2),0,0,0,"宋远桥");break;
					case 1:
						TASKREWARD_D->get_reward(ob,"武当",shen,0,0,5+random(2),0,0,0,"宋远桥");break;
					case 2:
						TASKREWARD_D->get_reward(ob,"武当",shen,0,0,7+random(2),0,0,0,"宋远桥");break;
					case 3:
						TASKREWARD_D->get_reward(ob,"武当",shen,0,0,8+random(2),0,0,0,"宋远桥");break;
					case 4:
						TASKREWARD_D->get_reward(ob,"武当",shen,0,0,9+random(2),0,0,0,"宋远桥");break;
					case 5..7:
						TASKREWARD_D->get_reward(ob,"武当",shen,0,0,10+ random(3),0,0,0,"宋远桥");break;
					case 8..9:
						TASKREWARD_D->get_reward(ob,"武当",shen,0,0,14+ random(3),0,0,0,"宋远桥");break;
					case 10:
						TASKREWARD_D->get_reward(ob,"武当",shen,0,0,50+ random(3),0,0,0,"宋远桥");break;
					default:
						TASKREWARD_D->get_reward(ob,"武当",shen,0,0,3+random(2),0,0,0,"宋远桥");
				}
			}
			...
			*/
			...
			//ob->clear_condition("wd_job"); 去掉wudang busy
...
string ask_me2()
{
	...
	for (i=0;i<sizeof(living);i++){
		...
		if( check(living[j]) ){
			...
			if( get_place(base_name(room))!="" && (string)base_name(room)+""!="/quest/smy/sys/memory" ) {
				oroom = room;
				...
				j = 1+random(4);//改得好找些:)
				for(k=0;k<j;k++) {
					if( !mapp(room->query("exits")) || sizeof(room->query("exits"))<1 ) break; //上来先判断room的exits属性，如果不存在出口就跳出这个小循环，继续大循环
					...
					if( !(room = find_object(exits[a])) )
						if( !(room = load_object(exits[a])) || ((string)base_name(room)+""=="/quest/smy/sys/memory" ))
							break;
				}
				if(k<j) continue;
				break;
