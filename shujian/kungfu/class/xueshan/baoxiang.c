...
string ask_finish()
{
	...
	if (me->query("xs/girl_super")) {
                   TASKREWARD_D->get_reward(me,"雪山",-1,0,2,47+ random(4),0,0,0,this_object());
      
                                                            }
	else
                    TASKREWARD_D->get_reward(me,"雪山",-1,0,0,20+random(3),0,0,0,this_object());
	...
	if ((string)me->query("family/family_name")=="大轮寺"&& me->query_skill("huanxi-chan",1) < 200)
	...
	log_file( "job/xueshan_done",sprintf("%8s%-10s第%5d次雪山任务得%3d点经验，%2d点潜能，经验：%d\n",me->name(),"("+me->query("id")+")",me->query("job_time/雪山"),exp,pot,me->query("combat_exp")),me);
...
string ask_job()
{
	...
	beauty->set_name(beauty->query("name"),beauty->parse_command_id_list()+({ me->query("id")+"'s beauty"  }) );
...
//	beauty->move("/d/xueshan/emptyroom");
	...
	ob->set_name(ob->query("name"),ob->parse_command_id_list()+({ me->query("id")+"'s guard"  }) );
...
string ask_fail()
{
	...
	if (userp(me))
	{
		my_girl=me->query("xs/girl_ob");
		if (my_girl&&living(my_girl))
		{
			my_guard=my_girl->query("guard");
			my_girl->start_call_out( (: call_other, my_girl, "dest":), 10);
			//my_girl->dest();
			if (my_guard&&living(my_guard))
			{
				//my_guard->dest();
				my_guard->start_call_out( (: call_other, my_guard, "dest":), 10);
			}
		}
	}
	...
	log_file( "job/xueshan_fail",sprintf("%8s%-10s放弃了第%5d次困难任务(%s)。\n",
