...
int main(object me, string arg)
{
	...
	j = sizeof(sname);
	if( j > 0){
		str += sprintf("%-96s", "│"HIY"『职业技能 "+chinese_number(j)+" 种』"NOR"：");
		for(i=0; i < j; i++) {
			if (i % 2 == 0) str += "│\n│";
			else str += "  ";
			str += sprintf("%s%-8s%-14s"HIC"→"NOR"%s %4d/%8d",
	...
	j = sizeof(sname);
	if( j > 0){
		str += sprintf("%-96s", "│"HIY"『杂项技能 "+chinese_number(j)+" 种』"NOR"：");
		for(i=0; i < j; i++) {
			if (i % 2 == 0) str += "│\n│";
			else str += "  ";
			str += sprintf("%s%-8s%-14s"HIC"→"NOR"%s %4d/%8d",
	...
	j = sizeof(sname);
	if(j > 0){
		str += sprintf("%-96s", "│"HIY"『基本技能 "+chinese_number(j)+" 种』"NOR"：");
		for(i=0; i < j; i++) {
			if (i % 2 == 0) str += "│\n│";
			else str += "  ";
			str += sprintf("%s%-8s%-14s"HIC"→"NOR"%s %4d/%8d",
	...
	if(j > 0){
		...
		for(i=0; i < j; i++) {
			if (strlen(strip(to_chinese(sname[i]), " (" + sname[i] + ")")) > 17) {
					str += sprintf("│  %s%s%-18s%-22s" HIC "→ "NOR"%s %4d/%8d %26s\n",
			(lrn[sname[i]] >= (skl[sname[i]]+1) * (skl[sname[i]]+1)) ? HIM : "",
		...
		skl[sname[i]], (int)lrn[sname[i]],"│");
			...
			str += sprintf("│  %s%s%-12s%-28s" HIC "→ "NOR"%s %4d/%8d %26s\n",
			(lrn[sname[i]] >= (skl[sname[i]]+1) * (skl[sname[i]]+1)) ? HIM : "",
			(member_array(sname[i], mapped)==-1? "  ": "□"),
			to_chinese(sname[i]), " (" + sname[i] + ")",
			skill_level(SKILL_D(sname[i])->type(), skl[sname[i]]),
