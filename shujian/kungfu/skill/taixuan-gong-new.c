...
//inherit FORCE;
//by nono buff主要方面
// 1、增加特殊状态下的auto反击（连击：效果1封招、剑法，掌法2hit，效果2解闭气、内功，效果3解busy：轻功），体现太玄功遇强则强的效果，符合原著（石破天领悟成功后与二位岛主切磋的描述）
// 2、增加太玄功的auto闪避效果，grin
// 3、buff 以下pfm xuan
// 4、新做pfm 融合 （剑、掌、内功、轻功攻击四合一）
// 。。。暂时这么多吧
inherit SKILL;
...
{
return usage == "cuff" || usage == "dodge" ||usage == "parry" || usage == "force" || usage == "strike"; || usage == "sword"; //符合原著，sword也能enable
}
...
mapping query_action(object me, object weapon)
{
	...
	enhance_level=(int)me->query("武功秘籍/"+skill_file);
...
if(!me->query("quest/taixuan/pass")) return 0; //防止bug
if( con < 49 && str < 49 && dex < 49 && j < 551) return 0;
					 
//自娱自乐的东西，随便吧，三叶能不能接受呢？

if(random(2) == 1 && me->is_busy()){ //busy 状态下有1/1的机会解除，grin
	   msg = HIW"$N行动刚一受限，石壁上的武功不由自主的使出，一气贯通，再不受到任何限制！\n"NOR;
       me->start_busy(-1);
     	}


if( me->query_condition("no_exert") && random(3) == 1){
               msg = HIY"$N发现自己无法自主呼吸，急忙暗运石壁上的内功心法，自己受损经络穴道当即恢复！\n"NOR;
               me->remove_condition("no_exert");
           } 
//if( me->query_condition("no_perform") && random(4) == 1){ //本来想写反震的，写不出来，直接调用pfm 算了
//			   msg = HIR"$N双掌竟然不由自主的直拍$n胸膛，用的竟是太玄经的功夫。\n"NOR;
