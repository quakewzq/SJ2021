	...
	if ( (int)me->query("combat_exp") <= 2000000 )
	...
	else
		{
			...
			if( family == "嵩山派")
			{
				...
				ss_target->set("long", ss_target->query("long")+"看来就是左盟主要求"+me->query("name")+"("+capitalize(me->query("id"))+")寻找的人。\n");
			...
			if (family == "丐帮")
			{
				...
				gb_target->set("long", gb_target->query("long")+"看来就是丐帮吴长老要求"+me->query("name")+"("+capitalize(me->query("id"))+")寻找的人。\n");
