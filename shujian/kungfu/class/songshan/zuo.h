...
string do_cancel()
{
	...
	ss_target=me->query_temp("quest/ss_target");
	if (ss_target)
	{
		//ss_target->dest();
		ss_target->start_call_out( (: call_other, ss_target, "dest":), 5);
	}
...
int accept_object(object who, object ob,object me)
{
       ...
       switch (type){
	    ...
	    default:command("haha");
			...
			TASKREWARD_D->get_reward(me,"嵩山",-1,0,0,8+random(3),0,0,0,me);
...
string do_ask_lingqi()
{
	...
	*/
	...
	object me=this_player() , lingqi;

	if (!me->query_temp("mz_entrance"))
		return 0;
	//lingqi = present("wuyue lingqi", this_object());
	lingqi = new(("d/meizhuang/obj/lingqi"));
	if (!clonep(lingqi))
		return "五岳令旗已经被别人拿走了,现在不在我手里。";
	
	//command("give wuyue lingqi to " + me->query("id"));
	lingqi->move(me);
	return "这五岳令旗就给你了！";
}
string do_ask_syt()
{
	object me=this_player() , tie;

	if (!me->query_temp("mz_entrance"))
		return 0;
	//tie = present("shuaiyi tie", this_object());
	tie = unew(("d/meizhuang/obj/shuaiyitie"));
	if (!clonep(tie))
		return "率意帖已经被别人拿走了,现在不在我手里。";
	if(clonep(tie) && tie->violate_unique())
	{
		destruct(tie);
		return "率意帖已经被别人拿走了,现在不在我手里。";
	}
