...
//							u_pfm = u_pfm + ({(: perform_action, "finger.qiankun" :)});     //弹指神通pfm：乾坤一指
							u_pfm = u_pfm + ({(: perform_action, "finger.shentong" :)});     //弹指神通pfm：弹
							switch(random(3))
							{
								case 0:
									set_skill("xuanfeng-tui",500);
				...
				case 1:
					switch(random(4))
					{
						case 0:
							...
							u_pfm = u_pfm + ({(: perform_action, "claw.lianhuan" :)}); 			//龙爪功pfm：风卷残云
...
//							u_w_pfm = u_w_pfm + ({(: perform_action, "sword.hundun" :)});
		...
		case "9y":
			...
			u_f_pfm=u_f_pfm + ({(: exert_function, "yihun" :)});   
			...
			u_f_pfm=u_f_pfm + ({(: exert_function, "xixing" :)});   //吸星大法pfm：吸星大法
					...
					switch(random(1))
					{
						case 0:
							...
							u_w_pfm = u_w_pfm + ({(: perform_action, "sword.muisc" :)});			//七弦黄龙闪	
							u_weapon="sword";
				...
				case 0:
					...
					switch(random(1))
					{
						case 0:
							...
							u_w_pfm=u_w_pfm + ({(: perform_action, "sword.fengwu" :)});          //天羽奇剑pfm：天羽狂舞
							u_weapon="sword";
				...
				case 1:
					...
					switch(random(1))
					{
						case 0:
							...
							map_skill("sword","tianyu-qijian");
							w_parry="tianyu-qijian";
							u_w_pfm = u_w_pfm + ({(: perform_action, "sword.fengwu" :)});			//闪电
              ...
              set_skill("zhemei-shou",500);								//逍遥高级空手：折梅手		
					...
					switch(random(1))
					{
						case 0:
							...
							w_parry="ruyi-dao";
							u_w_pfm=u_w_pfm + ({(: perform_action, "blade.ruyi" :)});          //如意刀法pfm：温柔一刀
							u_weapon="blade";
