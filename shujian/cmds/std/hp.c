...
int main(object me, string arg)
{
	...
	if (arg==getuid(me))
	{
		...
		if (!i) i = 1;
		j = ob->max_water_capacity();
		if (!j) j = 1;
		if (!my["max_jing"]) my["max_jing"] = 1;
		if (!my["max_qi"]) my["max_qi"] = 1;
		write(sprintf(HIC"HP:%d/%d/%d%%/%d/%d/%d/%d/%d/%d%%/%d/%d/\+%d/%d/%d/%d/%.2f%%/%d/%d/%.2f%%/%d/%.2f%%\n"NOR,
		
		my["jing"], my["eff_jing"]+ob->query_temp("apply/jing"),(my["eff_jing"]+ob->query_temp("apply/jing")) * 100 / my["max_jing"],
		
		my["jingli"], my["eff_jingli"]+ob->query_temp("apply/jingli"), my["eff_jingli"]-my["max_jingli"]+(ob->query_skill("force")-ob->query_temp("apply/force"))*8+ob->query_temp("apply/jingli"),
						
		my["eff_qi"]+ob->query_temp("apply/qi"), my["max_qi"], (my["eff_qi"]+ob->query_temp("apply/qi")) * 100 / my["max_qi"],
		
		my["neili"], my["max_neili"]+ob->query_temp("apply/neili"), my["jiali"],
