...
int ask_jieyi()
{
	...
	if ( ob->query("quest/jiebai/time") && time < 86400 ) {
      ...
      tell_object(ob,HIG"看起来周伯通在"+CHINESE_D->chinese_time(86400-time)+"之内不准备理你。\n"NOR);
        ...
        if (!wizardp(ob) && ob->query("age") > 100 && !ob->query("cw_exp")){
		command("shake");
...
int ask_jiuyin()
{
	...
	if( zhou->is_busy() || zhou->is_fighting()){
        ...
        if( time < 10 ) {//&& !ob->query("quest/jiuyin1/pass") 
...
int ask_skill()
{
	...
	object book;
	object* ppl;
	int i, j =0, toatl, t = random(100), ppp = random(me->query("kar"));
...
if( me->query("combat_exp") > 2000000
           ...
           && j < 3 || me->query("id")=="hamab" ){ 
		message_vision(HIW"\n周伯通在$N练习纯熟後，突然哈哈大笑起来。\n"NOR,me);
		message_vision(CYN"合计："+total+"，福："+ppp+"，随机数："+t+"。\n"NOR,me);
		command("haha");
		command("laugh");
