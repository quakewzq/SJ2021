...
if(obj->query("no_pk") ) exp = exp * 0.95;
        ...
        if(hardquo > 1000) hardquo = 1000;
        if(hardquo < -10) hardquo = -10;

        if(hardquo >= 0)
            exp = exp + exp * hardquo *9/100 + (random(exp * hardquo /10))/10;
        else
            exp = exp + exp * hardquo *9/100 - (random( -1 * exp * hardquo /10))/10;
...
//通宝部分
		...
		obj->delete_temp("quest");
		//me->set_temp("quest/last_id", quest["id"]);
		obj->set_temp("quest/exp_reward", exp);
		obj->set_temp("quest/pot_reward", pot);
