...
//          return notify_fail(HIR"\n你试着将真气灌注到"+wpn->query("weapon_mp/name")+""HIR"中。。。。\n"HIG"你沮丧的发现，"+wpn->query("weapon_mp/name")+""HIG"未曾经历过龙潭血祭，无法经受你的真气淬制。\n\n"NOR);
 ...
 if ( wpn->query("weapon_mp/weapon_prop/damage",1)      >=1200
	|| wpn->query("weapon_mp/weapon_prop/attack",1)      >=620
	|| wpn->query("weapon_mp/weapon_prop/dexerity",1)     >=140
	|| wpn->query("weapon_mp/weapon_prop/constitution",1) >=140
	|| wpn->query("weapon_mp/weapon_prop/intelligence",1) >=140
	|| wpn->query("weapon_mp/weapon_prop/strength",1)     >=140 )
    ...
    {  me->add_temp("con_weapon",1);
	  ...
	  call_out("thinking",1+random(2), me);
		...
		{  		 
			...
			if(random(10000)>=7)
				wpn->add("weapon_mp/weapon_prop/damage",d/5); 
			if(wpn->query("weapon_mp/weapon_prop/damage",1) >1200)  
				wpn->set("weapon_mp/weapon_prop/damage",1200) ;
			
			
				//命中值 a= wpn->query("weapon_mp/weapon_prop/attack",1);
			if(random(10000)>=7)
				wpn->add("weapon_mp/weapon_prop/attack",a/5);
			if(wpn->query("weapon_mp/weapon_prop/attack",1) >620)
				wpn->set("weapon_mp/weapon_prop/attack",620);
			
				//身法属性 dexe= wpn->query("weapon_mp/weapon_prop/dexerity",1);
			if(random(10000)>=7)
				wpn->add("weapon_mp/weapon_prop/dexerity",dexe/5);
			if( wpn->query("weapon_mp/weapon_prop/dexerity",1)>140)
				wpn->set("weapon_mp/weapon_prop/dexerity",140);
			
				//根骨属性 cons= wpn->query("weapon_mp/weapon_prop/constitution",1);
			if(random(10000)>=7)
				wpn->add("weapon_mp/weapon_prop/constitution",cons/5);
			if( wpn->query("weapon_mp/weapon_prop/constitution",1)>140)
				wpn->set("weapon_mp/weapon_prop/constitution",140);
			
				//悟性属性 inte= wpn->query("weapon_mp/weapon_prop/intelligence",1);
			if(random(10000)>=7) 
				wpn->add("weapon_mp/weapon_prop/intelligence",inte/5);
			if( wpn->query("weapon_mp/weapon_prop/intelligence",1)>140)
				wpn->set("weapon_mp/weapon_prop/intelligence",140);
			
				//臂力属性 stre= wpn->query("weapon_mp/weapon_prop/strength",1) ; 
			if(random(10000)>=7) 
				wpn->add("weapon_mp/weapon_prop/strength",stre/5) ;   
			if( wpn->query("weapon_mp/weapon_prop/strength",1)>140)
				wpn->set("weapon_mp/weapon_prop/strength",140);
		
			wpn->add("weapon_mp/dur",100) ;   
			wpn->add("weapon_mp/max_dur",100) ;   
			wpn->add("dur",100) ;   
			wpn->add("max_dur",100) ;   
			//wpn->add("sharpness",1) ;   
