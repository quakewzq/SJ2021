        ...
        {                       
                ...
                for(i=0;i< sizeof(dirs);i++)
                {
                        if( (!room_obj=find_object(exits[dirs[i]]))
							//&&(exits[dirs[i]]+"")!="/quest/smy/sys/memory" )
						)
...
int look_item(object me, object obj)
{
	...
	if ((int)obj->query("weapon_mp/upgrade_count",1)>0 )
	{
		write(HIY"这是一件"HIC+chinese_number(obj->query("weapon_mp/upgrade_count"))+HIY"级兵器。\n"NOR);
	}
	if ((int)obj->query("armor_mp/upgrade_count",1)>0 )
	{
		write(HIY"这是一件"HIC+chinese_number(obj->query("armor_mp/upgrade_count"))+HIY"级装备。\n"NOR);
	}
