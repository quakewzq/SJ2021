...
int perform(object me, object target)
{
    ...
    me->start_perform(1+random(3),"「琴剑双绝」");     	
    
    msg = BBLU+HIY "\n$N忽然仰天大喝一声"HIR"“琴 剑 双 绝”"HIY"，手上的剑如风车般连环击出，一阵热浪随剑势狂飙而出，势不可挡！\n"NOR;    	
    ...
    msg = HIG"$n看到剑光偏右，疾侧身左转，但只这一刹，$N的剑光刹时袭向左首！"NOR;
       COMBAT_D->do_attack(me,target, me->query_temp("weapon"),TYPE_REGULAR,msg);
    msg = HIY"$n看到剑光偏上，疾侧身下转，但只这一刹，$N的剑光刹时袭向下身！"NOR;
       COMBAT_D->do_attack(me,target, me->query_temp("weapon"),TYPE_REGULAR,msg);
    msg = HIR"$n看到剑光偏下，疾侧身上转，但只这一刹，$N的剑光刹时袭向上身！"NOR;
       COMBAT_D->do_attack(me,target, me->query_temp("weapon"),TYPE_REGULAR,msg);
    }

    msg = BBLU+HIY "$N姿势极为优雅的刺出一剑，看似及其平淡确是包含了极强的威力！\n"NOR;
