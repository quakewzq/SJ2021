...
int do_answer(string arg)
{
	...
	int level;
	object objkuang,objkuang2,fujia;
...
//  if(wizardp(me) && INTERMUD_MUD_NAME!="TEST" && INTERMUD_MUD_NAME!="SJTY") return notify_fail("你老人家不是一直用SMASH的么？\n");
	...
	if( !me->query("imbue_reward")){
        ...
        if (userp(me) && me->is_fight()) return notify_fail("你正忙着呢！\n");
		
		if (!arg ||	(sscanf(arg, "%s %s with %s", type, kuangcai, jade) != 3) ||(sscanf(arg, "%s %s with %s %d", type, kuangcai, jade,j) == 4))
			return notify_fail("指令格式：dazao <武器类型> <材料> with <玉>，玉不能使用yu 2或者yu 3等这种格式！\n");
        ...
        if(sscanf(kuangcai,"%s with %s",kuangcai,jade)==2)
        {
        	if ( !objectp(fujia = present(jade, me)) )
                return notify_fail("你身上没有打造要使用的宝玉。\n");
            if ( !objectp(objkuang = present(kuangcai, me)) )
                return notify_fail("你身上没有打造要使用的材料。\n");
             
            if(fujia->query("material_for")!="weapon") 
				return notify_fail("你左看右看总觉得"+fujia->name()+"与"+objkuang->name()+"不合适。\n");
                /*
        ...
        if ( !objectp(objkuang = present(kuangcai, me)) )
                return notify_fail("你身上没有打造要使用的材料。\n");
			objkuang = present(objkuang->query("id"), me);
        if ( !objectp(objkuang2= present(objkuang->query("id")+" 2", me)) )
                return notify_fail("你必须用两块"+objkuang->query("name")+"来锻造兵器。\n");
           
			/*
			switch( (int)objkuang->query_level() ) {
                case 1: break;
        ...
        if(objkuang && ((int)objkuang->query_level() > 0) )
        {
        	objkuang->move(npc);
        	id1= objkuang->query("id");
        	message_vision("$N交给$n一"+objkuang->query("unit")+objkuang->name()+"。\n",me,npc );
        }
        
        if(objkuang2 && ((int)objkuang2->query_level() > 0))
        {
        	objkuang2->move(npc);
        	
        	message_vision("$N交给$n一"+objkuang2->query("unit")+objkuang2->name()+"。\n",me,npc );
        }
        
        if(fujia && ((int)fujia->query_level() > 0))
        {
        	fujia->move(npc);
        	id2= fujia->query("id");
        	message_vision("$N交给$n一"+fujia->query("unit")+fujia->name()+"。\n",me,npc );
        }
        ...
        if(!npc->go_dazao(lu,me,cmds)){
			log_file("career/weilan_dazao",cmds+"       "+getuid(me)+" ask 打造失败。\n",this_object() );
        	log_file("career/worker",getuid(me)+" ask 打造失败。\n",this_object() );
...
string ask_weilan()
{
  ...
  //    if(wizardp(me)  && INTERMUD_MUD_NAME!="TEST" && INTERMUD_MUD_NAME!="SJ" ) return "你老人家不是一直用SMASH的么？\n";
	...
	if( !me->query("imbue_reward")){
        ...
        return "你想要打造什么就跟我说(dazao)吧！\n锻造(dazao)：dazao 兵器种类 原料 [with 佩宝]\n（现在可以锻造的兵器有：sword，xiao，axe，blade，brush，club，\ndagger，fork，hammer，hook，spear，arrow , staff，stick，whip）\n";
		//return "锻造(duanzao)：duanzao 兵器种类 原料 [with 佩宝]\n";
		//return "（现在可以锻造的兵器有：sword，xiao，axe，blade，brush，club，\n";
		//return "dagger，fork，hammer，hook，spear，arrow , staff，stick，whip）\n";
}
