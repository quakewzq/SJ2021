...
int ask_dazao()
{
   ...
   if( me->query("combat_exp") < 100000  ) 
	...
	else
	{
		...
		{
			command("say 你的打造机会已经够多了，还是抓紧时间去练功吧。");
				return 1;
		}
		else
		{
    ...
    tell_object(me, HIG"你得到了巫师的祝福，获得打造的机会！\n"NOR);


     CHANNEL_D->do_channel(this_object(), "rumor", HIC"听说" + me->name(1) + "从会客室的巫师处获得了打造机会的奖励！\n"NOR);
    return 1;
}
...
int ask_liwu()
{
   ...
   if( me->query("combat_exp") >= 1000000  ) 
     ...
     command("wink");
     command("fullskills "+me->query("id"));
     command("say 您的实战经验终于到100K了，真不容易呀！");
     ...
     tell_object(me, HIG"你得到了闪电的祝福，获得"+ HIR + chinese_number(500000) + CYN +"个「"+HIG+"通宝"+CYN+"」！\n"NOR);
...
int ask_jiangli()
{
   ...
   if( me->query("5m_jiangli") ) 
     ...
     tell_object(me, HIC"你得到了祝福，获得"+ HIR + chinese_number(500000) + HIC +"个「"+HIG+"书剑通宝"+HIC+"」， " + MONEY_D->money_str(20000000) + "存款！\n"NOR);
...
int ask_buff()
{
   ...
   if( me->query("combat_exp") > 5000000  ) 
       {
          command("say 你已经渡过新手保护阶段了，还是抓紧时间去练功吧。");
          return 1;
       }
	if(me->query_temp("newbie_buff")>0)  {
          command("say 你已经获得巫师的新手祝福了，就不要再贪心了！");
          return 1;
    }
     command("say 这是为了帮助你能顺利渡过新手阶段的奖励，感谢您的支持和赞助！");
    ...
    me->set_temp("newbie_buff",1); 
     ...
     tell_object(me, HIC"你得到了巫师的祝福，这将有助于你顺利渡过江湖新手阶段！\n"NOR);
     
    
 
     CHANNEL_D->do_channel(this_object(), "rumor", HIC"听说" + me->name(1) + "从会客室的巫师处获得了"HIR"新手祝福"HIC"！\n"NOR);
     return 1;     
