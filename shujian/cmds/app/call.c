...
int main(object me, string arg)
{
	...
	if (SECURITY_D->get_status(me) == "(apprentice)" && obj!=me)
		return notify_fail("apprentice 你没有直接呼叫这个物件的函数的权力。\n");

	if (wiz_level(me) < wiz_level(obj))
		return notify_fail("wiz_level(me<obj)你没有直接呼叫这个物件的函数的权力。\n");

	if (userp(obj) && geteuid(me) != "hongba") {
		...
		return notify_fail("master()->valid_wirte()你没有直接呼叫这个物件的函数的权力。\n");
