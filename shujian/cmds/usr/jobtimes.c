...
int main(object me,string arg)
{
        ...
        if (ob->query("job_time/"+job_type[j]["job_name"]))
                ...
                (intp(ob->query("job_last_gift/"+job_type[j]["job_name"])) && ob->query("job_last_gift/"+job_type[j]["job_name"])!=0 )?sprintf("%d 次",ob->query("job_last_gift/"+job_type[j]["job_name"])):HIG"0"NOR);
