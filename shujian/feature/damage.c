...
//added by snowman for some shadow npcs' hit
	...
	switch(type){
				...
				case "jing":  	damage_desc = "精血" ; break;
				case "jingli":	damage_desc = "精力" ; break;
				default: damage_desc = "气血";
			}
	...
	if (target)
	{
		if(me->is_fighting(target) && userp(me) && me->query("env/damage"))
			tell_object(me,WHT""+ target->query("name") +"的攻击对你造成了"RED+ damage + WHT"点"+damage_desc+"伤害。\n"NOR);
		if(target->is_fighting(me) && userp(target)&& target->query("env/damage"))
			tell_object(target,WHT"你对"+ me->query("name") +"造成了"RED+ damage+ WHT"点"+damage_desc+"伤害。\n"NOR);
	}
		*/
	if( val < 0 ) val = -1;
	set(type, val);
