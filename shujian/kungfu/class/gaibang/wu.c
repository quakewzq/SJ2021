...
void ok(object apper)
{
	...
	/* 参数说明：
			...
			*/
			TASKREWARD_D->get_reward(apper,"丐帮",0,0,0,15+random(3),0,0,0,this_object());
...
string ask_fangqi()
{
  ...
  //      me->add("job_time/丐帮", -1);
		gb_target=me->query_temp("quest/gb_target");
		if (gb_target)
		{
			//gb_target->dest();
			gb_target->start_call_out( (: call_other, gb_target, "dest":), 5);
		}
        me->start_busy(random(2));
