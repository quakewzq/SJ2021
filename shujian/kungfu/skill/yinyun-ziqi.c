...
if(!me->query("quest/wdjiuyang/pass") && !me->query_temp("quest/wdjiuyang/pass") ) return 0;
 ...
 if (random(dp)> ap*2 /3 && random(me->query("neili")) > ob->query("neili")/3 && lvl >550 )
	...
	if(userp(me) && me->query("env/damage"))      tell_object(me,WHT"你的反震对"+ ob->query("name") +"造成了"RED+ damage/10+ WHT"点伤害。\n"NOR);    
    if(userp(ob)&& ob->query("env/damage"))      tell_object(ob,WHT""+ me->query("name") +"的反震对你造成了"RED+ damage/10+ WHT"点伤害。\n"NOR);
              return i;                               
           }      
