...
varargs int skill_power(object ob, string skill, int usage)
{
	...
	switch(usage) {
		...
		case SKILL_USAGE_DEFENSE:
			...
			if (ob->is_fighting())
				level += level * to_int(ob->query_temp("fight/"+skill) / 100);
	}

	if( level < 1 )
		{
            power = to_int(ob->query("combat_exp") / 3600);
			if (power < 1) power = 1;
		}
				
	else
		{
            power = to_int((level*level*level)/3 + ob->query("combat_exp")/2) / 1000;
			if (power < 1) power = 1;
		}
...
if (!me->query_temp("weapon")) 	damage = (my_neili/200 + factor + factor*factor/200 - v_neili/300)/5;
	     //增加空手的内力加成。
else 	damage = (my_neili/250 + factor + factor*factor/400 - v_neili/300)/5;
...
*/
		...
		// Hit! modified by snowman
			...
			damage = me->query_temp("apply/damage")/10; //降低附加damage的初始伤害，防止伤害爆表
			if (weapon)
				damage += (me->query_temp("apply/damage") + weapon->query_weight()/2500)/2;
			else
				damage += me->query_temp("apply/damage")/2;

            if (damage > 2000) damage = 2000;
			damage = (damage*9/10 + random(damage/10)) / 2;
			if (action["damage"])
			...
			switch(attack_skill){
				...
				case "dagger":  em = -damage_bonus/9; break;
				case "brush":	em = -damage_bonus/8; break;
				case "sword":   em = -damage_bonus/8; break;
				case "whip":	em = -damage_bonus/5; break;
				case "blade":   em = -damage_bonus/7; break;
				case "staff":	em = -damage_bonus/6; break;
				case "club":	em = -damage_bonus/7; break;
				case "spear":	em = -damage_bonus/8; break;
				case "claw":    em =  damage_bonus/6; break;
				case "throwing":em = -damage_bonus/7; break;
				case "axe":		em = -damage_bonus/7; break;
				case "hammer":	em = -damage_bonus/6; break;
				case "hand":	em =  damage_bonus/8; break;
           	    case "strike":	em =  damage_bonus/7; break;
                case "cuff":	em =  damage_bonus/6; break;
                case "leg":		em =  damage_bonus/6; break;
			    case "finger":  em =  damage_bonus/8; break;
				default: em = 0;
			}
			damage_bonus += em;
...
//							foo /= 2;
			...
			if( action["force"] ) { //招式附加内功伤害通过兵器作用力要少于直接空手 faint 20180415
				if (weapon) 
					damage_bonus += action["force"] * damage_bonus / 350; 
				else 
					damage_bonus += action["force"] * damage_bonus / 250;
			}

			if (weapon)
				damage_bonus += me->query_skill("force") * damage_bonus / 48;
			else
			...
			*/
			...
			//if( damage_bonus < 0 ) damage += (damage_bonus - random(-damage_bonus))/2;			
			if( damage_bonus > 0 ) damage += (damage_bonus*19/10 + random(damage_bonus/10))/2;
			if( damage_bonus < 0 ) damage += (damage_bonus*19/10 - random(-damage_bonus/10))/2;			
			if( damage < 0 ) damage = 0;
			//faint 增加平砍伤害暂时注释掉
			/*
			...
			*/ 
			...
			#define TYPE_BUSY	4
			*/
			if (attack_type == TYPE_REGULAR)  damage = damage*1; 
			if (attack_type == TYPE_BUSY)  damage = damage*8/10; 
			if (attack_type == TYPE_QUICK) damage = damage*11/10;
            if (attack_type == TYPE_RIPOSTE) damage = damage*12/10; 
			//update by lsxk@hsbbs 敌方是玩家，大幅度降低super威力.  2007/10/29
            if (attack_type == TYPE_SUPER)
			{
                //if(!userp(me) || !userp(victim)) //这个判断有点问题啊，me是攻击方，victim是对方
				if(userp(me) && !userp(victim)) //改成攻击方是玩家并且防御方不是玩家
					damage = damage*15/10;
				else 
					damage *= 1;
            }
			...
			message("wizard",sprintf( GRN "attack_type计算之后的Damage=%d。\n",damage),env );
			/*if (!weapon && vweapon)  //空手对阵武器
					
					damage = to_int(damage * (me->query_str(1)*(1+(me->query_str()-me->query_str(1))/me->query_str()) ) / (victim->query_con(1)*(1+(victim->query_con()-victim->query_con(1))/victim->query_con()) ));
			else 
					damage = to_int(damage * (me->query_str(1)*(1+(me->query_str()-me->query_str(1))/me->query_str()) ) / (victim->query_dex(1)*(1+(victim->query_dex()-victim->query_dex(1))/victim->query_dex()) ));
			*/
			if (!weapon && vweapon)
			 	   //damage = to_int(damage * (me->query_str(1) / 5.0 + 16) / (victim->query_con(1) / 4.0 + 15));
				damage=to_int(damage*(me->query_str()*6/10+me->query_int()*2/10+me->query_con()/10+me->query_dex()*2/10)/(victim->query_str()*3/10+victim->query_int()*2/10+victim->query_con()*4/10+victim->query_dex()/10));
...
//杀人或者打昏者任一一人身上有复仇标记
	...
	if(multi) {
	 ...
	 if(killer->query("revenge/"+victim->query("id")))             killer->add("PKS", -1);	
					...
					str += sprintf("被%s%s死了。", killer->short(1), vmark);
				}
			}
			if (multi)
