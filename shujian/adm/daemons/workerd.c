...
object load_user_object(object me,mapping u_map)
{
	...
	if ( imbued_object_type(u_map["save_id"])=="weapon" ) {
		...
		/*
		for (i=0;i<sizeof(w_map);i++)
		{
			//weapon_des+=w_map[i]+" ";
		}
		*/
		//log_file("weapon_des", sprintf("%s\n",weapon_des), me);
		...
		if( weapon ) {
			...
			//if (weapon->query("weapon_mp/upgrade_count")<1 ) {
				//log_file("weapon_upgrade_times", sprintf("%-8s  升级武器次数为：%s \n",me->query("id"),weapon->query("weapon_mp/upgrade_count"),ctime(j)), me);
				//weapon->set("weapon_mp/upgrade_count",0);
			//}
			weapon->move(me);
		}
		return weapon;
...
void check_user_object(object me)
{
	...
	if( strlen(log) > 3 )
		log_file("career/worker", sprintf("login   :%-8s:%s\n",me->query("id"),log), me);
...
mapping convert_armor2user(mapping mp)
{
	...
	my_mp["upgrade_count"]	=	mp["upgrade_count"]; //升级次数
...
object make_weapon(object me, mixed type, int qua, string material,mapping shuxing,string* status)
{
	...
	if( w_mp["imbued"] < 5 && p > 180000000000000000 && !new_w ){
...
object make_armor(object me,mixed type,int qua,string material,mapping shuxing,string* status)
{
	...
	if( wp->armor_setup(copy(w_mp)) ){
		...
		wp->set("long" , str);
		if(w_mp["armor_type"]==TYPE_ARMOR) wp->set("male_only",1);
...
protected mapping make_user_armor_random(object me,string type, int qua, string material,mapping shuxing,string* status)
{
	...
	*/
	...
	else
	{
		...
		//mp["maker"] += "  " + me->name(1) + "(" + capitalize(me->query("id")) + ")";
		mp["maker"] = "韦兰铁匠" + "(Weilan tiejiang)";
	}

	mp["save_id"] = sprintf("%s%da","weilan tiejiang",time());
	return mp;
}
